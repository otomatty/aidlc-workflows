...
- いまの安定版は **2.8.2**（タグ `v2.8.2`、`upstream/main` の先端 `a0ee4415`）です。
  版番号はコミット済みのフレームワーク木を指します。GitHub Releases の Latest は `v2.8.2` です。
- AI-DLC Workflows 2.0 は既定ブランチ `main` で **GA** です。新規導入とアップグレードは `main` を使います。以前の実装は `v1` で別管理です。
- ネイティブバイナリ、インストーラ、版の選択、プロジェクト設定、リリースの出自は #722 と #756 で出荷済みです。
...
| 2.7.0 - 2.8.0 | ネイティブバイナリ、インストーラ、プロジェクト設定、版の選択、更新、タグ結びのリリース出自 | - | #756, #993, #1050 |
| 2.7.1 - 2.8.2 | Plan Approval と Change Control の強化、ハーネスをまたぐネイティブフック修正、設定ウィザード修正、stable / preview リリース経路 | 1, 3, 4 | #997, #1000, #1054, #1064, #1065, #1067, #1008, #1097, #1111, #1127, #1129 |
