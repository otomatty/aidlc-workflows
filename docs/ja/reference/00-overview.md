...
> **このリファレンスのパス。** AI-DLC は一度書いてハーネスごとに生成するので、ファイルの呼び方は意図で四つに分かれます。
> - **`core/…`** — 手で書く、ハーネス非依存の**正本**（例: `core/tools/aidlc-orchestrate.ts`、`core/aidlc-common/stages/`）。直すのはここです。*どこで書いたか / どこを変えるか* を言うときは `core/` です。
> - **`dist/<harness>/…`** — **無視されるローカル生成**のソース / 開発投影（`dist/claude/.claude/`、`dist/kiro/.kiro/`、`dist/kiro-ide/.kiro/`、`dist/codex/`、`dist/opencode/`、`dist/copilot/`）。手では触らず、コミットしません。`bun scripts/package.ts` が実体化します。リリースのペイロードは `aidlc-runtime-X.Y.Z.tar.gz` 内の `runtime/<harness>/` です。
> - **`<harness-dir>/…`**（例: `.claude/`、`.kiro/`、`.codex/`） — *入れた*プロジェクト内の**実行時**の場所。コマンドが走り、ワークフロー中にフレームワークが読み書きします（`{{INVOKE}} engine graph compile`、`loadAgents()` が `.claude/agents/` を読む）。ディレクトリはハーネスのパラメータです。
>
...
| [Hooks and Tools](06-hooks-and-tools.md) | フック、CLI ツール、98 種の監査イベント |
| [Sensor System](07-sensor-system.md) | センサーマニフェストのスキーマ、PULL import、発火模型、既定の重大度 |
...
| [State Machine](12-state-machine.md) | ワークフロー / フェーズ / ステージ機械、98 種の分類、監査先行の規則 |
...
| [Supply-Chain Security](19-supply-chain-security.md) | リリース証明、SLSA provenance、チェックサム、ワークフローの硬化、インストーラの防御、所有、エンタープライズ輸送 |
| [Commit Provenance](20-commit-provenance.md) | コミットからレビュー済み Unit への内容由来の帰属。コミットしたレビュー済みソースの証拠、`aidlc attest resolve`、`SOURCE_COMMITTED` アンカー |
...
- **成果物を足す、または改名する？** [Artifact Vocabulary](16-artifact-vocabulary.md) です。命名規則、安定方針（改名・削除は major、追加は minor）、現行一覧は `aidlc engine graph artifacts`。レジストリはステージファイルから派生し、手では書きません。
