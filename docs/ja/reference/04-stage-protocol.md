...
| # | 確認 |
|---|-------|
| 1 | 承認ゲートでは `aidlc engine orchestrate report --stage <slug> --result awaiting-approval` を呼ぶ。ゲート結びのセンサーは、既存の成果物ごとに一度、トランザクションの前に走る。blocking の結びは、検証済みの通過が要る。オーバーライドするなら、別の `Fix findings` / `Override blocking sensors` 決定をログして見せ、人が裏打ちした正確な答えを待ち、`--override-blocking-sensors --user-input "Override blocking sensors"` で再試行する。裸のフラグと自律モードは拒否される。エンジンは状態を `[-]` から `[?]` AwaitingApproval へ回し、`STAGE_AWAITING_APPROVAL` を原子的に出すので、プロンプトが開いているあいだステータスはゲート待ちを示す。（`STAGE_STARTED` / `[-]` への遷移は、ステージがアクティブになったときに出ている。） |
...
| 6 | ステージ末尾では、ゲート付きなら `aidlc engine orchestrate report --stage <slug> --result approved --user-input "<exact choice>"`、Initialization なら `report --stage <slug> --result completed` を呼ぶ。エンジンは `[?]` / `[-]` を `[x]` へ回し、ゲート付きなら `GATE_APPROVED` を出し、状態ツール経由で `STAGE_COMPLETED` を原子的に出す |
...
| 9 | ライフサイクルイベントを手書きせず、`aidlc-state.ts` にライフサイクル動詞も呼ばない。結果は `aidlc engine orchestrate` 経由で報告する。エンジン内部の状態呼び出しが、原子的な監査行を出す |
...
- まとめた答えの要約を出し、そのあと構造化した **Looks correct** / **Request changes** 確認の前に `aidlc-review-brief.ts summary --stage <slug> --questions-file <path>` を印字する。決定論的なブリーフはステージ、質問ファイル、生成した成果物、いま決める理由、両選択肢の正確な効果を名指しする。確認を裸の散文で聞かない。出す前に、ステージの質問ファイルへ専用の **Consolidated Summary Confirmation** エントリを追記またはリセットし、両選択肢と空の `[Answer]:` を付ける。プロンプトは `aidlc engine log decision --checkpoint summary-confirmation --questions-file <path>` で記録し、人で止まり、正確な選択を書き、対になる `aidlc engine log answer` で記録する。レシートは人の手番を、正確な質問ファイルダイジェストへ結ぶ。**Request changes** では **「What should change?」** を聞き、どの答えも直す前にもう一度止まる。フィードバックと直しのあと、確認を空に戻してから出し直す。それ以外の返信は提示した選択肢に合わないと認め、同じ手番で有効な 2 つを出し直し、タグもレシートも書かない。
...
**各承認ゲートで:** (1) `AskUserQuestion` の前 — `awaiting-approval` を報告。(2) 応答のあと — 正確な利用者入力付きで `approved` または `rejected` を報告。report 所有のライフサイクルイベントがゲートの完全な監査記録です。`aidlc engine log decision` も `aidlc engine log answer` も呼ばない。

**ゲート以外の各質問のやり取りで:** 答えを受け取ったあと — `aidlc engine log answer` で Q&A 要約を追記する。
...
1. **起動。** ディスパッチのたびに — 最初、NOT-READY の再起動、第 0 部のゲート差し戻し改訂のあとの再レビュー — コンダクターは先にレビュー依頼を記録します。ロガーは宣言した成果物すべてを、安定したファイル身元スナップショット 1 つで捉え、依頼をそれらのバイトちょうどと、該当すればワークスペースとユニットごとのソースフィンガープリントへ結び、`Request Id` を発行し、JSON で `requestId` と `reviewFile` を返します。`reviewFile` は、この依頼のレビューが書かれる、インテントレコードの `.aidlc-reviews/` ディレクトリ下のプロジェクト相対パスです。依頼がそのスロットを開きます（同じ反復の先行する未完了ディスパッチが残した下書きは消えます）。ディレクティブの `review_artifact` フィールドが、レビューが何についての必須 Markdown 出力かを指名します。レコードはそれに鍵付けされ、ゲートがそれを指名し、所見セレクタがそれを指します。出力順もプラグイン追加もそれを変えられず、レビュー中に誰もそれに書きません。再ディスパッチでは、コンダクターは先に `aidlc-review-brief.ts context --stage <slug>`（該当すれば `--unit` も）を走らせ、水を入れた所見を先行レビュー文脈として残し、それから `directive.reviewer` が指名するエージェントへ委譲し、レビュアーが書く 1 ファイルとして `reviewFile` パスを渡します。依頼が未対応のあいだ、ゲートと完了は止まります。レビュアーはステージ定義パス、Q&A ファイル、出した成果物パス、frontmatter の検証ツールを受け取ります — ビルダーの `memory.md` や計画は渡さないので、独立した判断になります。再試行は元の成果物 / ソース結びとリクエスト id を使い、いまのバイトで基線を付け直しません。古いレシート復旧のあいだレビュー凍結はオンのままです。レビュアーは成果物の隣へ書き、中へは書かないので、書き込み窓は要りません。
2. **レビュー。** `adversarial` レビューは敵対レビュー契約の下で走ります。レビュアーは成果物を確認するのではなく覆そうとし、機械検査できる証拠があるところでは所見をそこに根ざします（READY は届かない判定であり、既定ではありません）。`advisory` レビューは証拠に根ざす規則は残しますが、通常フローの意思決定支援を 1 通しです。所見はゲートで人向けに重大度順になり、後ろに修理ループはありません。どちらもレビュアーは定義、Q&A、成果物を読み、列挙された検証ツールがあれば走らせ、書くファイルはちょうど 1 つ — `reviewFile` パスのレビュー — です。レビューは一致する Verdict、Reviewer、Iteration 行を 1 組、所見表、2 つ目の H2 節は無しです。レビュアーはほかに何も書かず、特にレビュー対象の成果物には書きません。依頼はディスパッチ前に成果物バイトとワークスペースソースを結びます。再試行はどちらも基線を付け直せず、完了は安定したファイル身元スナップショット 1 つを使います。レビュアーは硬いターン上限（`maxTurns: 60`）の下で走ります。ペルソナ frontmatter に一度書き、ハーネスにレバーがあるところでネイティブに強制します。Claude Code はキーをそのまま読む（サブエージェントはタスク途中で止まり、最終メッセージの手番は無い）。opencode パッケージャはネイティブのエージェント単位 `steps: 60` へ投影します（ランナーは最後のテキスト専用手番を 1 つ許す — 要約は返せるが、ツール呼び出しでレビューは書けない）。Codex TOML ペルソナ、Cursor、Copilot、Kiro CLI/IDE はエージェント単位の上限キーを出さないので、そこでは予算はペルソナ散文だけです（ペルソナの `## Turn Budget` 節は、どのハーネスでも最悪の切断を見込んでいます）。
3. **判定と決定ブリーフ。** コンダクターは同じ `aidlc-log.ts review` コマンドに `--verdict` を付けて判定を記録します。ロガーは依頼の `reviewFile`（または `--review-file <path>`）からレビューを読み、検証し、ディスパッチした成果物バイトと依頼時のソース身元が変わっていないことを証明し、レビューレコード `<record>/.aidlc-reviews/<stage>/stage/<attempt>/<iteration>.json` または `<record>/.aidlc-reviews/<stage>/units/<unit>/<attempt>/<iteration>.json`（判定、所見、レビュアー、リクエスト id、成果物とソースのフィンガープリント、レビュー本文）を、それを指名してダイジェストをピンする `REVIEW_COMPLETED` 行と同じロックしたトランザクションで書きます。レコードを書くのはこのコマンドだけです。あとで直したレコードは行へハッシュしなくなり、レビューではありません。
   `advisory` では、通常フローで両判定とも終端です。ワークフローは学びの儀式とゲートへ進みます。そのゲートの前に、`aidlc-review-brief.ts review --stage <slug> --why <first|revision|stale>` が正確なステージ、普段の言葉での結果、レビュー成果物、レコードから水を入れた所見、決定の効果、具体的な上流 / 下流の無効化経路を描きます（`reviewer_max_iterations` は 1、エンジンが強制）。ユニットごとのステージの最終ゲートは、その 1 回の承認が覆うすべてのユニットを描きます。ユニットの絞り込みはレビュアーディスパッチ文脈に限ったままです。
   ...
   判定が数に入るのは、レビューファイルが、一致する正本身元フィールド付きのレビューちょうど 1 つとしてパースできるときだけです。ファイルが無い（上限やクラッシュで止まったレビュアーは書かずに止まる — 依頼は空スロットを開いたので、残った下書きが代用にはならない）、正本の判定行が無いレビュー、重複した判定は未完了試行です。コンダクターは同じ未対応依頼を `--retry-pending` で一度再試行します（回数は消費しない — advisory の通常フロー予算はちょうど 1 通しなので、数えた切断はレビュー無しで使い切る）。2 回目の未完了試行は終端レシート `--verdict NOT-READY` を、レビューファイル無しで、ブリーフのフォールバック所見「review did not complete within its turn budget」付きで記録します — ゲートは具体的な所見付きで到達し、静かに欠けた判定の上（またはそれで行き詰まったまま）では出ません。
   `adversarial` で回数が残っていれば、再起動はリードを飛ばします（成果物はレビューされていない。ビルダーが動く対象が無い）。
   レビュー依頼が通るのは、まとめた答えが確認済みで、検証できる必須出力文書がすべてあり、解決できる権威ユニット集合を持つユニットごとのステージが、その集合に存在するユニットだけを名指しするときです。解決できない集合では依頼は拒否しません。名指しユニットの出力は必須のまま、ステージ単位の依頼は検証できない全ユニット出力の列挙を飛ばします。解決済み集合を持つユニットごとのステージへのステージ単位依頼は、権威あるすべてのユニットを覆うので、各ユニットの該当必須出力が要ります。記録したレシートは、そのあとにレビュー通しが続かないときは終端です。あとから出力文書を書くと、レビューはいまの文書を覆わなくなるので、直しは反復ループの中で行い、終端レシートのあとではしません。
...
レビューレコードが存在する前に記録したレビューは、`review_artifact` 内の終端 `## Review` 節として住みます。それらの節は読めるままです。スコープにレコードが無いとき、ゲートブリーフと再ディスパッチ文脈がそれらを描き、Plan Approval 投影は計画から 1 つをまだ剥ぎます。まだ 1 つを追記するレビュアーは、このリリース周期だけ許されます（非推奨）: ロガーは、依頼より後であることが証明できるときその節を判定として受け入れ、検証した節をレビューレコードへコピーし、次の minor で埋め込み入力フォームを外します。プロトコルは新しい埋め込み節を書きません。古い節は惰性の中身として残ります。
...
2. **提示**: `aidlc engine learnings surface --slug <slug>` が日記を読み、構造化した候補を出す — LLM は再パースも分類もしない。
...
5. **残す**: `aidlc engine learnings persist` が確認した学びをプラクティスとして `aidlc/spaces/<active-space>/memory/{project,team}.md` へ書く（センサー結びの学びなら、マニフェスト + ステージの `sensors:` import を 1 つのロック付きトランザクションで入れる）。`RULE_LEARNED` / `SENSOR_PROPOSED` を出す。
