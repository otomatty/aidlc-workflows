...
技術環境ドキュメントは、プロジェクトの構築方法を規定する**技術的なツール、標準、制約、および設定方針**を定義します。ビジョンドキュメントの技術的な対となるものであり、AI-DLCのConstruction(コンストラクション)フェーズにおける拘束力のある参照ドキュメントとして機能します。

このドキュメントにより、コード生成・インフラ設計・非機能要件の判断が、組織の標準、セキュリティポリシー、チームの能力と一致していることが保証されます。これがなければ、AI-DLCの各ステージは不足情報を補うために多くの確認質問を行うか、さらに悪い場合には手戻りを要する仮定を行う可能性があります。

## 技術環境ドキュメントを書くタイミング

- 新規プロジェクトを開始する前（グリーンフィールド）
- 技術的な制約が変更された既存プロジェクトを修正する前（ブラウンフィールド）
- 組織の技術標準が更新されたとき
- クラウドプロバイダー、フレームワーク、またはデプロイモデルを移行するとき

## ドキュメントの適用範囲

技術環境ドキュメントは、以下のいずれかのプロジェクトコンテキストを対象にできます。

| コンテキスト       | 定義                                                 | 主な違い                                                                               |
| ------------------ | ---------------------------------------------------- | -------------------------------------------------------------------------------------- |
| **グリーンフィールド** | 既存コードなし。ゼロから構築する。                   | すべての選択肢が開かれています。ドキュメントが出発点を定義します。                     |
| **ブラウンフィールド** | 既存のコードベースあり。追加・修正・移行を行う。     | 選択肢は既存のものによって制約されます。ドキュメントは何を保持・変更・回避するかを定義します。 |

該当するコンテキストに合わせてドキュメントを構成してください。以下のセクションは、適用範囲を示すために **(グリーンフィールド)**、**(ブラウンフィールド)**、または **(両方)** と表記されています。
...
プロジェクトで使用しなければならない言語、使用してもよい言語、使用してはならない言語を定義します。
...
### 8. サンプルコードとテンプレートコードのガイダンス（両方）

このセクションでは、プロジェクトの規約を確立するサンプルコードやテンプレートコードを、AI-DLCおよび開発チームがどのように用意・使用・維持するかを定義します。
...
| 技術環境セクション                   | AI-DLCステージ                                   | 活用方法                                                   |
| ------------------------------------ | ------------------------------------------------ | ---------------------------------------------------------- |
| プロジェクト技術サマリー             | ワークスペース検出                               | プロジェクト分類のためのコンテキスト                       |
| プログラミング言語                   | コード生成                                       | 言語選択とバージョン制約                                   |
| フレームワークとライブラリ           | コード生成、非機能要件設計                       | 依存関係の選択と禁止ライブラリのチェック                   |
| クラウドサービス許可・禁止リスト     | インフラ設計                                     | サービス選択の範囲の決定                                   |
| 推奨パターン                         | アプリケーション設計、機能設計                   | アーキテクチャと設計パターンの判断                         |
| セキュリティ要件                     | 非機能要件の要件定義、非機能要件設計             | セキュリティパターンの選択とコンプライアンスチェック       |
| テスト要件                           | コード生成、ビルドとテスト                       | テスト戦略、ツール、カバレッジ目標                         |
| サンプルコード                       | コード生成                                       | コード生成時のパターン参照                                 |
| ブラウンフィールドインベントリ       | リバースエンジニアリング、ワークフロー計画       | 移行判断と共存ルール                                       |
